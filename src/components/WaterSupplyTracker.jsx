          ...
          <div key={entry.id || index} className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
            <div>
              <div className="flex items-center gap-2 text-slate-500 mb-1">
                <Calendar size={14} />
                <span className="text-[10px] font-bold uppercase tracking-wider">Start Date & Time ({entry.label})</span>
              </div>
              <p className="font-bold text-sm text-slate-800 dark:text-slate-100">{formatDateTime(entry.start)}</p>
            </div>
            <div>
              <div className="flex items-center gap-2 text-slate-500 mb-1">
                <Calendar size={14} />
                <span className="text-[10px] font-bold uppercase tracking-wider">End Date & Time ({entry.label})</span>
              </div>
              <p className="font-bold text-sm text-slate-800 dark:text-slate-100">{formatDateTime(entry.end)}</p>
            </div>
            <div className="bg-blue-50/50 dark:bg-blue-900/10 px-3 py-2 rounded-lg border border-blue-100/50 dark:border-blue-900/30">
